<?php
   ...
   $sql="SELECT * FROM people WHERE country = '대한민국' AND ssn='".$_POST['ssn']."' AND (hospitalization_hospital = '영남대병원' OR hospitalization_hospital is NULL)";
   $ret=mysqli_query($con, $sql);
   if($ret){
   $count = mysqli_num_rows($ret);
   if($count==0) {
      echo $_POST['ssn']." 해당 ssn의 국민이 존재하지않습니다."."<br>";
      echo "<br> <a href='yu_select.php'><--초기화면</a>";
      exit();
      }
     }
   else{
...
<FORM METHOD="post" ACTION="yu_update_result.php">
   ...
   입원병원:<INPUT TYPE ="text" NAME="hospitalization_hospital" VALUE=<?php echo '영남대병원' ?> READONLY><br>
