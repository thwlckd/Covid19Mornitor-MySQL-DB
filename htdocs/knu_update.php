<?php
   ...
   $sql="SELECT * FROM people WHERE ssn='".$_POST['ssn']."' AND (hospitalization_hospital = '경북대병원' OR hospitalization_hospital is NULL)";
   $ret=mysqli_query($con, $sql);
   if($ret){
   $count = mysqli_num_rows($ret);
   if($count==0) {
      echo $_POST['ssn']." 해당 ssn의 국민이 존재하지않습니다."."<br>";
      echo "<br> <a href='knu_select.php'><--초기화면</a>";
      exit();
      }
     }
   else{
...
<FORM METHOD="post" ACTION="knu_update_result.php">
   ...
   입원병원: <INPUT TYPE ="text" NAME="hospitalization_date" VALUE=<?php echo '경북대병원' ?> READONLY> <br>
